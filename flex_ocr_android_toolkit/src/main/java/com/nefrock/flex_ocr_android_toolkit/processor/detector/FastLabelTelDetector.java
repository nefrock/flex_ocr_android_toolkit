...
public class FastLabelTelDetector implements Detector {
    ...
    public FastLabelTelDetector(Context context, String modelPath, Size size) {
    ...
    public DetectorResult process(Mat image, FlexScanOption option) {
        ...
        List<Detection> labels = NMS.nms(image, labelBboxes, labelScores, this.labelClassPredThre, this.nmsThre, FlexScanResultType.INVOICE_LABEL, false);
        List<Detection> tels = NMS.nms(image, telBBoxes, telScores, this.telClassPredThre, this.nmsThre, FlexScanResultType.TELEPHONE_NUMBER, true);
        labels.addAll(tels);
