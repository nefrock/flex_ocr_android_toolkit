...
public class FlexGeneralRecognizer implements Recognizer {
    ...
    public FlexGeneralRecognizer(Context context, String modelPath, Size inputSize) {
    ...
    public void process(Mat mat, Detector detector, FlexScanOption option, OnScanListener<FlexScanResults> listener) {
        ...
        for (Detection detection : detections) {
            ...
            Imgproc.cvtColor(imgResized, imgResized, Imgproc.COLOR_RGB2GRAY);
            imgData.rewind();
            for (int i = 0; i < inputY; ++i) {
                for (int j = 0; j < inputX; ++j) {
                    int pixelValue = (int) imgResized.get(i, j)[0];
    ...
    public void init() {
        try {
            ...
            outputPutValues = new float[NUM_BATCHES][48][CHARS.length() + 1];
